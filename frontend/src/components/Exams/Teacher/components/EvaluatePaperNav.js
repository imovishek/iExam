...
const QuestionCard = ({
  ...
  return (
    <TableRowStyled
      ...
      onClick={() =>
        dispatch(push(`/exam/${exam._id}/paper/question/${question._id}`))
      }
...
const EvaluatePaperNav = ({
  ...
  useEffect(() => {
    if (questionID) {
      setShowingStudentType(QUESTIONS);
    }
  }, [questionID]);
  ...
  const renderTable = () => {
    if (showingStudentType !== QUESTIONS)
      return (
        <TableWrapper>
    ...
    return (
      <TableWrapper>
        ...
        <Body>
          {_.map(exam.questions, (question, index) => (
            <QuestionCard
              dispatch={dispatch}
              key={`question_${index}`}
              question={question}
    ...
    <Container rows="32px 1fr">
      <Row columns="1fr 1fr">
        ...
        >
          ...
          <Option key={QUESTIONS} value={QUESTIONS}>
            Questions ({(exam.questions || []).length})
          </Option>
        </StyledSelect>
        {showingStudentType !== QUESTIONS && (
          <SearchStyled
            allowClear
            placeholder="Search"
...
export default connect(null, mapDispatchToProps)(EvaluatePaperNav);
