...
exports.mapCsvToCourse = (courses = [], user) =>
  courses.map((course) => {
    ...
    if (
      _.any(
        requiredHeaders,
        (header) => !_.contains(Object.keys(course), header)
      )
    )
  ...
  Promise.all(
    students.map(async (student) => {
      ...
      if (
        _.any(
          requiredHeaders,
          (header) => !_.contains(Object.keys(student), header)
        )
      )
        throw new Error("Some fields are missing");
  ...
  Promise.all(
    teachers.map(async (teacher) => {
      ...
      if (
        _.any(
          requiredHeaders,
          (header) => !_.contains(Object.keys(teacher), header)
        )
      )
        throw new Error("Some fields are missing");
      const salt = await bcrypt.genSalt(10);
...
exports.cleanExamForStudent = (
  req,
  exam,
  shouldDeleteQuestions = false,
  keepPaper = false
) => {
  ...
  if (
    status !== RUNNING &&
    (status !== ENDED || exam.shouldNotSeePaperAfterEnded)
  ) {
    exam.questions = [];
  }
  if (!keepPaper) exam.papers = [];
  else cleanPaperForStudent(req.user._id, exam);
};

exports.cleanExamForTeacher = (
  req,
  exam,
  shouldDeleteQuestions = false,
  keepPaper = false
...
const cleanPaperForStudent = (studentID, exam) => {
  exam.papers = _.filter(
    exam.papers,
    (paper) => String(paper.student) === String(studentID)
  );
};
