...
const getExamByIDWithParticipants = async (id) => {
  ...
  let participants = _.filter(
    ...
    (st) =>
      !_.any(
        result.bannedParticipants,
        (pt) => String(pt._id) === String(st._id)
      )
  );
  result.participants = _.filter(participants, (pt) =>
    _.any(result.papers, (paper) => String(pt._id) === String(paper.student))
  );
...
exports.getExamByIDWithUserPaper = async (req, res) => {
  ...
  try {
    const result = await getExamByIDWithParticipants(id);
    const papers = _.filter(
      result.papers,
      (paper) => String(paper.student) === studentID
    );
    ...
    if (question) {
      ...
      await Promise.all(
        _.map(result.papers, async (resPaper) => {
          const answers = _.filter(
            resPaper.answers,
            (answer) => String(answer.questionID) === question
          );
          const studentObj = await studentHelper.getStudentByID(
            resPaper.student
          );
...
exports.updateExamPaperForStudent = async (req, res) => {
  ...
  try {
    ...
    const exams = await examHelper.getExamAggregate(id, {
      startDate: 1,
      startTime: 1,
      duration: 1,
      bannedParticipants: 1,
    });
    const status = getExamStatus(exams[0]);
    const amIBanned = exams[0].bannedParticipants.filter(
      (bannedID) => String(bannedID) === req.user._id
    )[0];
    if (status === ENDED) throw new Error("Exam ended can't take submission");
    if (amIBanned)
      throw new Error("You got banned, please contact your course teacher");
    await Promise.all(
      _.map(paper.answers, async (answer) => {
...
exports.updateExamPaperForTeacher = async (req, res) => {
  ...
  if (req.user.userType !== "teacher")
    throw new Error("User requested is not authorized!");
  try {
    const exam = await examHelper.getExamByID(id);
    const questionsObj = {};
    _.forEach(
      exam.questions,
      (question) => (questionsObj[String(question._id)] = true)
    );
    const groupedAnswers = _.groupBy(paper.answers, (answer) => answer.paperID);
    await Promise.all(
      _.map(groupedAnswers, async (answers, paperID) => {
        const actualPaper = await paperHelper.getPaperByID(paperID);
        answers.forEach((answer) => {
          if (!questionsObj[answer.questionID]) return;
          const oldAnswer = actualPaper.answers.filter(
            (oldAnswer) =>
              String(oldAnswer.questionID) === String(answer.questionID)
          )[0];
          oldAnswer.marks = answer.marks;
        });
        actualPaper.totalMarks = _.reduce(
          actualPaper.answers,
          (sum, answer) => {
